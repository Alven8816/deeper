#' Tuning the parameters to get the best model
...
#' learner <- tuningModel("SL.ranger", params=list(num.trees=1000),
#' tune = list(mtry = c(1,3,7)))
#'
#' ##ensemble created models
#' predictModel(Y = trainSet[,dependence],X = trainSet[,predictor],
...
tuningModel <- function(basemodel = "SL.ranger",
        ...
        learners = SuperLearner::create.Learner(base_learner = basemodel,
                                                params = params,
                                                env = .GlobalEnv,
                                                tune = tune,...)
